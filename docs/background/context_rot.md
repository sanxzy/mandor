...
[![Video thumbnail](/img/context_rot/video_thumbnail.jpeg)](https://www.youtube.com/watch?v=TUjQuC4ugak)
...
![Context Rot: How Increasing Input Tokens Impacts LLM Performance](/img/context_rot/header_plot.png)

![Claude Sonnet 4, GPT-4.1, Qwen3-32B, and Gemini 2.5 Flash on Repeated Words Task](/img/context_rot/hero_plot.png)

Claude Sonnet 4, GPT-4.1, Qwen3-32B, and Gemini 2.5 Flash on Repeated Words Task

Recent developments in LLMs show a trend toward longer context windows, with the input token count of the latest models reaching the millions. Because these models achieve near-perfect scores on widely adopted benchmarks like Needle in a Haystack (NIAH) \[[1](/context-rot#niah)\], it’s often assumed that their performance is uniform across long-context tasks.
...
![](/img/context_rot/niah_lexical.png)
...
Additionally, we include a conversational question-answer evaluation using LongMemEval \[[2](/context-rot#longmemeval)\], as well as a synthetic task in which models replicate a series of repeated words. Each task remains intentionally simple and is deliberately controlled to isolate the impact of context length alone.
...
It is common for modern LLMs to have input context lengths in the millions of tokens. Gemini 1.5 Pro \[[3](/context-rot#gemini-1.5-pro)\] first introduced their 1M context window in early 2024, followed by the recent GPT-4.1’s 1M context window \[[4](/context-rot#gpt-4.1)\] and Llama 4 with 10M \[[5](/context-rot#llama-4)\]. The use case for long context is compelling: longer context means that the LLM can process more information with each call and generate more informed outputs.
...
However, NIAH underestimates what most long context tasks require in practice. Variants of NIAH, like NoLiMa \[[6](/context-rot#nolima)\] which include needle-question pairs with non-lexical matches, reveal significant performance drops. Other tasks that appear similar in regards to difficulty, such as AbsenceBench \[[7](/context-rot#absencebench)\] which tests models for recognizing the absence of a given snippet of text, also demonstrate performance degradation with growing input length.

Additionally, long context tasks often involve disambiguating amongst distractors as part of the task. One example is Multi-round co-reference resolution (MRCR) \[[8](/context-rot#michelangelo)\] \[[9](/context-rot#oai-mrcr)\], which involves retrieving the i-th instance of a specific user ask, amongst similar user asks, in a multi-turn conversation. However, there remains a lack of investigation into the impact of distractors in long context settings.

An important factor in long-context tasks is how input length is scaled. Latent List \[[8](/context-rot#michelangelo)\] is a task in which the model must perform a fixed number of Python list operations across various input lengths. Various ways to fill irrelevant context are tested, which reveal non-uniform impact on model performance \[[1](/context-rot#clarification)\]. For instance, adding list operations that locally cancel each other out degrades model performance more significantly compared to adding print statements. This highlights how the type of 'irrelevant content' matters, as some may introduce increasing complexity with input length.

Similarly, Graphwalks \[[10](/context-rot#graphwalks)\] is a graph traversal task in which the model is given a directed graph composed of hexadecimal hashes, then asked to perform breadth-first search starting from a random node. Increasing input length means increasing the size of the graph to traverse through, which increases task difficulty as a result. It is difficult to disambiguate increasing task complexity from input length, which makes it difficult to isolate the impact on performance due to input length alone. This points to the importance of isolating input length as the variable of interest, which is essential for understanding of how LLMs actually behave with long inputs.
...
![](img/context_rot/niah/lexical_2.png)
...
Models often have to deal with distractors as well, which has been shown to degrade performance \[[11](/context-rot#distractors)\].

Throughout this report, we distinguish between distractors and irrelevant content:

![](img/context_rot/niah/distractor_irrelevance.png)
...
We evaluate each model across its maximum context window with temperature=0 unless that setting is incompatible (i.e. o3) or explicitly discouraged (i.e. Qwen’s “thinking mode”). For Qwen models, we apply the YaRN method \[[13](/context-rot#yarn)\] to extend from 32,768 to 131,072 tokens.
...
3.  Use UMAP \[[14](/context-rot#umap)\] for dimensionality reduction with the following parameters: n\_neighbors=30, min\_dist=0.05, n\_components=50, random\_state=42
    
4.  Use HDBSCAN \[[15](/context-rot#hdbscan)\] to create clusters with the following parameters: min\_cluster\_size=10, min\_samples=15
...
![](img/context_rot/longmemeval/needle_question_sim_arxiv.png)
...
![](img/context_rot/niah/distractors_var.png)
...
![](img/context_rot/niah/distractors_num.png)
...
![](img/context_rot/niah/distractors_ind.png)
...
![](img/context_rot/niah/hallucinations.png)
...
![](img/context_rot/niah/needle_haystack.png)
...
![](img/context_rot/niah/needle_haystack_2.png)
...
![](img/context_rot/niah/structure_2.png)
...
![](img/context_rot/longmemeval/ex.png)
...
![](img/context_rot/longmemeval/claude.png)
...
![](img/context_rot/longmemeval/gpt.png)

LongMemEval Results - GPT Family

![](img/context_rot/longmemeval/gemini.png)

LongMemEval Results - Gemini Family

![](img/context_rot/longmemeval/qwen.png)
...
![](img/context_rot/longmemeval/claude_comparison.png)
...
![](img/context_rot/repeated_words/claude_line_plot.png)
...
![](img/context_rot/repeated_words/claude_position_accuracy.png)
...
![](img/context_rot/repeated_words/claude_num_words.png)
...
![](img/context_rot/repeated_words/gpt_line_plot.png)
...
![](img/context_rot/repeated_words/gpt_4_turbo.png)
...
![](img/context_rot/repeated_words/gemini_line_plot.png)
...
![](img/context_rot/repeated_words/qwen_line_plot.png)
...
![](img/context_rot/appendix/arxiv_pg_nq_sim.png)

Needle-Question Similarity - arXiv haystack/PG essay needles

![](img/context_rot/appendix/pg_pg_nq_sim.png)

Needle-Question Similarity - PG essay haystack/PG essay needles

![](img/context_rot/appendix/pg_arxiv_nq_sim.png)
...
![](img/context_rot/appendix/gpt_position_accuracy.png)

Repeated Words: Position Accuracy - GPT Family

![](img/context_rot/appendix/gemini_position_accuracy.png)

Repeated Words: Position Accuracy - Gemini Family

![](img/context_rot/appendix/qwen_position_accuracy.png)

Repeated Words: Position Accuracy - Qwen Family

![](img/context_rot/appendix/wc_gpt.png)

Repeated Words: Word Count Difference - GPT Family

![](img/context_rot/appendix/wc_gemini.png)

Repeated Words: Word Count Difference - Gemini Family

![](img/context_rot/appendix/wc_qwen.png)

Repeated Words: Word Count Difference - Qwen Family

[Try Chroma Cloud](https://trychroma.com/signup)

Chroma is the open-source search and retrieval database for AI applications.

[About](https://docs.trychroma.com/about)

[Logos](https://trychroma.notion.site/Chroma-Brand-Guidelines-ca583f361d7f4298853933499564333f)

[Careers](https://careers.trychroma.com/)

[Contact Us](mailto:hello@trychroma.com)

[Privacy](https://www.trychroma.com/privacy)

[Terms of Use](https://www.trychroma.com/terms)
