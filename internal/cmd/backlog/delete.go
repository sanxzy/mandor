...
func NewDeleteCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "delete <id>",
		Short: "Delete a backlog",
		Long:  "Delete a backlog (soft delete by default, hard delete with --hard).",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !input.DryRun && !input.Yes && input.Hard {
				...
				fmt.Fprintf(out, "All backlog files will be removed.\n\n")
			...
			result, err := svc.DeleteBacklog(input)
			if err != nil {
				return err
			}
	...
	cmd.Flags().BoolVar(&hardDelete, "hard", false, "Permanently delete backlog and all files")
